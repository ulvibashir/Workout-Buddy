...
export default function RunningPage() {
  const [runs, setRuns] = useLocalStorage<RunLog[]>("run_logs", [
    {
      id: "marathon_2026",
      date: "2026-05-03",
      distance: 43.08,
  ...
  const addRun = () => {
    ...
    setRuns([...runs, run].sort((a, b) => a.date.localeCompare(b.date)));
    setShowAdd(false);
  ...
  const avgPaceNum = runs.filter((r) => r.avgPace).length;
  ...
  const monthlyChart = Object.entries(monthlyData).map(([month, km]) => ({ month: month.slice(5), km: Math.round(km * 10) / 10 }));
  ...
  const paceData = runs
    ...
    .map((r) => {
      ...
      return { date: r.date.slice(5), pace: (min * 60 + (sec || 0)) / 60 };
    });
    ...
    <div style={{ backgroundColor: "#0f0f1a", minHeight: "100vh", paddingBottom: 90 }}>
      <div style={{ padding: "20px 16px 12px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <button
          ...
          style={{ backgroundColor: "#0077b6", border: "none", borderRadius: 10, padding: "8px 14px", color: "#fff", fontWeight: 600, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}
        >
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 12 }}>
          {[
            ...
            { label: "Runs Logged", value: runs.length, unit: "", color: "#0f9b58" },
        ...
        {paceData.length > 1 && (
          <div style={{ backgroundColor: "#1a1a2e", borderRadius: 14, padding: 14, marginBottom: 12 }}>
            <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>Pace Progression (lower = faster)</p>
            <p style={{ fontSize: 11, color: "#9ca3af", marginBottom: 10 }}>min/km (runs 5km+)</p>
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={paceData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#2d2d4e" />
                <XAxis dataKey="date" tick={{ fill: "#9ca3af", fontSize: 9 }} />
                <YAxis domain={["auto", "auto"]} tick={{ fill: "#9ca3af", fontSize: 9 }} />
                <Tooltip contentStyle={{ backgroundColor: "#1a1a2e", border: "1px solid #2d2d4e", borderRadius: 8 }} formatter={(v) => { const n = Number(v); return [`${Math.floor(n)}:${String(Math.round((n % 1) * 60)).padStart(2, "0")}/km`]; }} />
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {sortedRuns.map((run) => (
            <div key={run.id} style={{ backgroundColor: "#1a1a2e", borderRadius: 12, padding: 14, marginBottom: 8, borderLeft: "3px solid #0077b6" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>{run.distance}km</div>
                  <div style={{ fontSize: 12, color: "#9ca3af" }}>{run.date} · {run.runType}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>{run.avgPace}/km</div>
        ...
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.7)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
          <div style={{ backgroundColor: "#1a1a2e", width: "100%", borderRadius: "16px 16px 0 0", padding: 20, maxHeight: "85vh", overflowY: "auto" }}>
            ...
            {[
              ...
              { field: "avgPace", label: "Avg Pace (min/km)", type: "text", placeholder: "9:06" },
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select
                ...
                style={{ width: "100%", padding: "10px 12px", background: "#242442", border: "1px solid #2d2d4e", borderRadius: 8, color: "#fff", fontSize: 15 }}
              >
                {["Zone 2 Run", "Long Run", "Interval", "Race", "Easy Run", "Trail Run"].map((t) => (
