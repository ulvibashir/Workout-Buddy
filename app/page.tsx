  ...
  return (
    <div style={{ backgroundColor: "var(--bg)", minHeight: "100vh", paddingBottom: 40 }}>
      ...
      <div style={{ padding: "24px 24px 0" }}>
        <p style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 2 }}>{format(today, "EEEE, MMM d")}</p>
        <h1 style={{ fontSize: 26, fontWeight: 800, margin: 0 }}>{profile.name}</h1>
        <p style={{ color: "var(--text-muted)", fontSize: 12, marginTop: 2 }}>{profile.goals.primary}</p>
        <div style={{ marginTop: 12, padding: "10px 14px", backgroundColor: "var(--surface)", borderRadius: 10, borderLeft: "3px solid #e94560" }}>
          <p style={{ color: "var(--text-muted)", fontSize: 12, fontStyle: "italic", margin: 0 }}>"{quote}"</p>
        </div>
      </div>
      ...
      <div style={{ padding: "14px 24px 0" }}>
        ...
        {todayWorkout && (
          <Link href="/workout" style={{ textDecoration: "none" }}>
            <div style={{ backgroundColor: "var(--surface)", borderRadius: 14, padding: 16, marginBottom: 12, borderLeft: `4px solid ${todayWorkout.color}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <p style={{ fontSize: 11, color: "var(--text-muted)", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: 0.8 }}>Today's Session</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
          {[
            ...
            <div key={label} style={{ backgroundColor: "var(--surface)", borderRadius: 12, padding: "12px 10px", textAlign: "center" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
          {[
            ...
            <div key={label} style={{ backgroundColor: "var(--surface)", borderRadius: 12, padding: "12px 10px", textAlign: "center", borderTop: `3px solid ${color}` }}>
        ...
        <div style={{ backgroundColor: "var(--surface)", borderRadius: 14, padding: 14, marginBottom: 12 }}>
          <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.8 }}>This Week</p>
        ...
        {recentRuns.length > 0 && (
          <div style={{ backgroundColor: "var(--surface)", borderRadius: 14, padding: 14, marginBottom: 12 }}>
            ...
            {recentRuns.map((run) => (
              <div key={run.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                <div>
                  <span style={{ fontSize: 13, fontWeight: 600 }}>{run.distance}km</span>
                  <span style={{ fontSize: 11, color: "var(--text-muted)", marginLeft: 8 }}>{run.date}</span>
                </div>
                <div style={{ textAlign: "right" }}>
                  {run.avgPace && <span style={{ fontSize: 13, color: "#0077b6", fontWeight: 600 }}>{run.avgPace}/km</span>}
                  {run.avgHR > 0 && <span style={{ fontSize: 11, color: "var(--text-muted)", marginLeft: 8 }}>{run.avgHR}bpm</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        {recentPullUps.length > 0 && (
          <div style={{ backgroundColor: "var(--surface)", borderRadius: 14, padding: 14, marginBottom: 12 }}>
            ...
            {recentPullUps.map((session) => (
              <div key={session.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>{session.id}</span>
                <div>
                  <span style={{ fontSize: 13, color: "#e94560", fontWeight: 700 }}>{session.maxReps} max</span>
                  <span style={{ fontSize: 11, color: "var(--text-muted)", marginLeft: 8 }}>{session.totalReps} total</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ backgroundColor: "var(--surface)", borderRadius: 14, padding: 14, marginBottom: 12, borderTop: "3px solid #fd7e14" }}>
        ...
        {/* Read-only notice */}
        <div style={{ padding: "10px 14px", backgroundColor: "var(--surface)", borderRadius: 10, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
          <TrendingUp size={14} color="var(--text-muted)" />
          <p style={{ color: "var(--text-muted)", fontSize: 11, margin: 0 }}>Dashboard is read-only. Log workouts & data from the iOS app.</p>
        </div>
      </div>
