...
import { WORKOUTS, DAY_LABELS } from "@/lib/data/workouts";
...
export default function Dashboard() {
  ...
  const [metrics, setMetrics] = useLocalStorage<Record<string, QuickMetrics>>("body_metrics", {});
  const [showQuickLog, setShowQuickLog] = useState(false);
  const [form, setForm] = useState<QuickMetrics>({ weight: "", rhr: "", hrv: "" });
  const todayKey = getTodayKey();
  const todayMetrics = metrics[todayKey];

  const [nutritionLog] = useLocalStorage<Record<string, { calories: number; protein: number; carbs: number; fat: number }>>(
    "nutrition_totals", {}
  );
  ...
  const recentDays = Object.entries(metrics)
      ...
      <div style={{ padding: "20px 16px 0" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <button
            ...
            style={{
              backgroundColor: "#e94560", borderRadius: "50%", width: 40, height: 40,
      ...
      <div style={{ padding: "16px" }}>
        ...
        {todayWorkout && (
          <Link href="/workout" style={{ textDecoration: "none" }}>
            <div style={{
              backgroundColor: "#1a1a2e", borderRadius: 14, padding: 16, marginBottom: 12,
              borderLeft: `4px solid ${todayWorkout.color}`,
            }}>
          ...
          <Link href="/nutrition" style={{ textDecoration: "none" }}>
            <div style={{ backgroundColor: "#1a1a2e", borderRadius: 14, padding: 14, marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
                {[
                  { label: "Calories", value: todayNutrition.calories, color: "#fd7e14" },
                  { label: "Protein", value: `${todayNutrition.protein}g`, color: "#e94560" },
                  { label: "Carbs", value: `${todayNutrition.carbs}g`, color: "#0077b6" },
                  { label: "Fat", value: `${todayNutrition.fat}g`, color: "#20c997" },
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: isToday ? w.color : `${w.color}33`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 9, fontWeight: isToday ? 700 : 400,
                    color: isToday ? "#fff" : w.color,
                    border: isToday ? "none" : `1px solid ${w.color}44`,
                  }}
                >
          ...
          <div style={{
            ...
            <div style={{ backgroundColor: "#1a1a2e", width: "100%", borderRadius: "16px 16px 0 0", padding: 20 }}>
              ...
              {[
                ...
                <div key={field} style={{ marginBottom: 12 }}>
                  ...
                  <input
                    ...
                    style={{
                      width: "100%", padding: "10px 12px", background: "#242442", border: "1px solid #2d2d4e",
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                <button onClick={() => setShowQuickLog(false)} style={{ flex: 1, padding: 12, background: "#242442", border: "none", borderRadius: 10, color: "#9ca3af", cursor: "pointer", fontSize: 14 }}>
                  Cancel
                </button>
                <button onClick={saveMetrics} style={{ flex: 2, padding: 12, background: "#e94560", border: "none", borderRadius: 10, color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>
                  Save
                </button>
              </div>
            </div>
