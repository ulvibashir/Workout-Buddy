      ...
      <div style={{ padding: "0 16px" }}>
        ...
        <div style={{ backgroundColor: "#1a1a2e", borderRadius: 14, padding: 16, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 12 }}>
            ...
            <div style={{ width: 1, backgroundColor: "#2d2d4e" }} />
        ...
        <div style={{ marginBottom: 12 }}>
          <p style={{ fontSize: 12, color: "#9ca3af", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>
            Daily Meal Plan
          </p>
          {meals.map((meal) => (
        ...
        <div style={{ backgroundColor: "#1a1a2e", borderRadius: 14, padding: 14, marginBottom: 16 }}>
          ...
          {supplements.map((sup, i) => (
            <div key={sup.id}>
              ...
              {i < supplements.length - 1 && <div style={{ height: 1, backgroundColor: "#2d2d4e" }} />}
            </div>
          ))}
        </div>

        {/* Cheat Day Note */}
        <div style={{ backgroundColor: "#1a1a2e", borderRadius: 14, padding: 14, marginBottom: 16, borderLeft: "3px solid #e94560" }}>
          <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 6 }}>🎉 Cheat Day — Sunday</p>
          <p style={{ fontSize: 12, color: "#9ca3af", marginBottom: 8 }}>You've earned it. Keep it reasonable.</p>
...
function MealCard({ meal }: { meal: NutritionMeal }) {
  ...
  return (
    <div style={{ backgroundColor: "#1a1a2e", borderRadius: 12, marginBottom: 8, overflow: "hidden" }}>
      <button onClick={() => setExpanded(!expanded)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: 14, display: "flex", alignItems: "center", gap: 12, textAlign: "left" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "#9ca3af" }}>{meal.time}</div>
        </div>
        {expanded ? <ChevronUp size={16} color="#9ca3af" /> : <ChevronDown size={16} color="#9ca3af" />}
      </button>
      {expanded && (
        <div style={{ padding: "0 14px 14px", borderTop: "1px solid #2d2d4e" }}>
...
function ExpandableFoodSection({ header, color, items }: { header: string; color: string; items: { name: string; stat: string }[] }) {
  ...
  return (
    <div style={{ backgroundColor: "#1a1a2e", borderRadius: 12, marginBottom: 8, overflow: "hidden" }}>
      ...
      {open && (
        <div style={{ padding: "0 14px 12px", borderTop: "1px solid #2d2d4e" }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: i < items.length - 1 ? "1px solid #2d2d4e" : "none" }}>
              <span style={{ fontSize: 13 }}>{item.name}</span>
              <span style={{ fontSize: 11, color: "#9ca3af" }}>{item.stat}</span>
            </div>
          ))}
        </div>
      )}
